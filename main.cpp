...
void balloons(int x[],int y[])
{
	for(int i=0;i<12;i++)
	{
		...
		if(y[i]<-200) y[i]=700;
		settextstyle(2,0,5); setbkcolor(13); setcolor(15); outtextxy(x[i]-40,y[i],"Happy Birthday Như Phương");
	}
}
int main()
{
	...
	while(1)
	{
		settextstyle(6,0,20);setcolor(12); setbkcolor(0); outtextxy(80,200,"Happy Birthday Như Phương");
		balloons(x,y);
		delay(100);
	}
}
